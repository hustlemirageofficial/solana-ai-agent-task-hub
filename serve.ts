...
import { ensureSchema } from "./src/server/schema.ts";

// Apply the schema idempotently at boot (no-op when DATABASE_URL is unset — the
// server must boot in graceful demo mode). Fire-and-forget: API handlers await
// the same cached promise, so the first DB call can't race the migration.
ensureSchema().catch((err: unknown) => {
  console.warn(
    "[agentpay] schema setup skipped:",
...
for (let attempt = 1; ; attempt++) {
  ...
  try {
    Bun.serve({
      port: PORT,
      hostname: HOST,
      // Bun's default idleTimeout is 10s, but approve/reject run on-chain
      // confirm loops (10–150s) after signing. Without a longer timeout the
      // client's fetch dies with "socket connection closed unexpectedly" even
      // though the handler completes in the background (DB ends consistent).
      // 120s covers the slowest confirm loop with margin; deposit/run never
      // come close to it.
      idleTimeout: 120_000,
      async fetch(req) {
        const { pathname } = new URL(req.url);
        // JSON API first — this TanStack Start version no longer ships the
        // src/routes/api/* mechanism (createAPIFileRoute), so the REST layer is
        // mounted here, ahead of static files and the TanStack handler.
