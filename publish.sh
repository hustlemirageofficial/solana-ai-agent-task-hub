...
# Keep bun's package cache off the tiny /home volume as well.
export BUN_INSTALL_CACHE_DIR="${BUN_INSTALL_CACHE_DIR:-/opt/bun-cache}"
mkdir -p /opt/bun-cache

# node_modules lives on the overlay filesystem (/opt/site-node_modules) because
# /home is a 300M volume and this dependency tree needs ~350M. It's bind-mounted
# onto site/node_modules so runtime resolution (node/bun walk realpaths) works.
# A machine restart drops bind mounts, so re-create it if the marker is missing
# before installing/building. Fall back to a plain install if the overlay copy
# is gone (e.g. someone wiped /opt) — that still works, just slower to fill /home.
if [ -d /opt/site-node_modules ] && [ ! -f node_modules/.bun-tag ]; then
  sudo mount --bind /opt/site-node_modules node_modules 2>/dev/null || true
fi
